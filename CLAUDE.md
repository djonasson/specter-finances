...
- **A payment whose amount varies** (`amountVaries`) stores no amount, not-counted included: the figure is unknown until the bill arrives. The generator emits blanks, and `RecurringPrompt` refuses to write the row until someone types one — a `€0` row is worse than no row, because it settles as though the bill were free. The rule's form hides the money fields when the box is ticked and clears them, so unticking cannot resurrect a stale figure.
