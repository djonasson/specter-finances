    ...
    function draw(time: number) {
      ...
      for (let i = icicles.length - 1; i >= 0; i--) {
        ...
        if (ic.growing) {
          ...
          const icicleCanScare =
            squirrel.invincible <= 0 && squirrel.mood !== 'flattened' && squirrel.mood !== 'love';
      ...
      if (squirrel.mood !== 'neutral' && squirrel.moodTimer > 0) {
        ...
        // 'love' draws a heart via drawSquirrel, so it shows no speech bubble
        if (squirrel.mood !== 'love') {
          let text: string;
          if (squirrel.mood === 'happy' && wasRescued) {
            text = 'Tszankyou!';
          } else if (squirrel.mood === 'happy') {
            text = 'Tszi!';
            wasRescued = false;
          } else if (squirrel.mood === 'flattened') {
            text = '...';
          } else if (squirrel.mood === 'anxious') {
            text = '!!!';
          } else {
            text = 'Nontszifacoszi!';
            wasRescued = false;
