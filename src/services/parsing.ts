...
/**
 * True when a string is a well-formed YYYY-MM-DD calendar date.
 *
 * Dates that fail this still reach the UI as-is (the raw cell is never
 * discarded), but they must not take part in date comparisons: string ordering
 * would happily rank "March 2013" above "2025-01-01" and pull it into a
...
export function normalizeDate(raw: unknown): string {
  ...
  if (dmy) {
    // Shape alone is not enough: "02/13/2025" (a US-format paste) matches but
    // would yield the impossible "2025-13-02". Fall through and warn instead.
  ...
  if (dmy2) {
    ...
    const candidate = `${year}-${dmy2[2].padStart(2, '0')}-${dmy2[1].padStart(2, '0')}`;
    if (isIsoDate(candidate)) return candidate;
  }
  if (str) {
